...
MM.Item.prototype.getUserid = function() {
    return this._userid;
}
...
MM.Map.prototype.getUserid = function() {
    return this._root.getUserid();
}
...
MM.Backend.WebDAV.load = function(url, data) {
	return this._request("get", url, data);
}

MM.Backend.WebDAV.list = function(url, data) {
    return this._request("get", url, data);
}

MM.Backend.WebDAV._request = function(method, url, data) {
	var xhr = new XMLHttpRequest();
	url = "http://127.0.0.1:8000/api/v0/mindmap/message"
...
MM.UI.Backend._error = function(e) {
	MM.App.setThrobber(false);
	alert("IO error in my-mind: " + e.message);
}
...
MM.UI.Backend.WebDAV.init = function(select) {
	...
	this._id = this._node.querySelector("#_id");
...
MM.UI.Backend.WebDAV.save = function() {
    ...
    jqr_mindmap_save(url, data);  //call jquery version
...
MM.UI.Backend.WebDAV.load = function() {
	this._load(this._url.value, this._id.value);
}

MM.UI.Backend.WebDAV._load = function(url, _id) {
	url = 'http://127.0.0.1:8000/api/v0/mindmap/message'
	this._current = url;
	MM.App.setThrobber(true);
	...
	data = _id
    jqr_mindmap_load(url, data);
	// this._backend.load(url, data).then(
	// 	this._loadDone.bind(this),
	// 	this._error.bind(this)
	// );
}
...
MM.UI.Backend.WebDAV.list = function() {
    this._list(this._url.value, this._id.value);
}

MM.UI.Backend.WebDAV._list = function(url, _id) {
    url = 'http://127.0.0.1:8000/api/v0/mindmap/message'
    this._current = url;
    MM.App.setThrobber(true);
...
MM.UI.Backend.WebDAV.delete = function() {
    this._delete(this._url.value, this._id.value);
}

MM.UI.Backend.WebDAV._delete = function(url, _id) {
...
//javascript
j_function_save = null;
j_function_load = null;
j_function_delete = null;
j_function_list= null;

//jquery
$(function() {

    // SAVE
    function my_fun_save(url, data){

		type = $('#Type').val();
		storyid = $('#Storyid').val();
		senddata = type + ":" + storyid + ":" + data;
        senddata = JSON.stringify(encodeURIComponent(senddata));  // encodeURIComponent 특수문자도 url 인코딩해야 함 http://rosalife.tistory.com/48
        var jqxhr = $.post( "http://127.0.0.1:8000/api/v0/mindmap/message", senddata,  function(data, status) {
            //alert("return_Data: " + data + "\nStatus: " + status);
            //window.history.pushState('page2', 'Title', '/editor?mapid='+ data);
            $('#Mindmap').val(data['mindmap']);
    ...
    j_function_save = my_fun_save;


    // DELETE
    function my_fun_delete(url, data){
    ...
    j_function_delete = my_fun_delete;


	// LOAD
    function my_fun_load(type, id){

        // encodeURIComponent 특수문자도 url 인코딩해야 함 http://rosalife.tistory.com/48
		senddata = type+":"+id;
        var jqxhr = $.get( "http://127.0.0.1:8000/api/v0/mindmap/message", senddata,  function(data, status) {
    ...
    j_function_load = my_fun_load;


	// LIST
    function my_fun_list(url, data){

        data = data;  // encodeURIComponent 특수문자도 url 인코딩해야 함 http://rosalife.tistory.com/48
		url = "http://127.0.0.1:8000/api/v0/storylist/list"
...
//just function mindmap save
function jqr_mindmap_save(url, data) {
    j_function_save(url, data); //== call jquery function - just Reference is globally defined not function itself
}

//just function mindmap load
function jqr_mindmap_load(url, data) {
    j_function_load(url, data); //== call jquery function - just Reference is globally defined not function itself
}

//just function mindmap delete
function jqr_story_delete(url, data) {
    j_function_delete(url, data); //== call jquery function - just Reference is globally defined not function itself
}

//just function mindmap load
function jqr_story_list(url, data) {
    j_function_list(url, data); //== call jquery function - just Reference is globally defined not function itself
}
